...
public class MainActivity extends AppCompatActivity
    ...
    private void updateUi(News news) {
        ...
        dateTextView.setText(getDateString(news.date));

        // Display whether or not there was a news alert about fires in the UI
        TextView authorTextView = (TextView) findViewById(R.id.author);
        authorTextView.setText(getFireAlertString(news.author));
    }
    ...
    private String getFireAlertString(int newsAlert) {
        switch (newsAlert) {
            ...
            default:
                return getString(alert_not_available);
        }
    }
    ...
    private class NewsAsyncTask extends AsyncTask<URL, Void, News> {
         ...
         * about the first fire from the input firesJSON string.
         */
        private News extractFeatureFromJson(String firesJSON) {
            try {
                ...
                if (featureArray.length() > 0) {
                    ...
                    // Extract out the title, time, and fire values
                    String title = properties.getString("title");
                    long time = properties.getLong("time");
                    int newsAlert = properties.getInt("news");
